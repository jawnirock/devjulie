  ...
  <div class="branding container" >
      ...
      <div class="branding-item-images" :class="{ fadeIn: fadeIn }" v-for="(item, index) in items" :key="'image-' + index">
          <transition-group name="component-fade" mode="out-in">
            <img v-for="img in splitJoin(item.image)" v-if="activeItem == index" :src="'http://julie.pash.rocks' + img" :key="img" v-on:load="onLoaded(img)">
          </transition-group>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      loadItems: function () {
        var app = this
        axios.get('http://julie.pash.rocks/api/branding').then(function (response) {
